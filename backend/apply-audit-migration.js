...
async function applyAuditMigration() {
  ...
  try {
    ...
    // Check if audit_events migration already applied
    ...
    if (applied.rows.length > 0) {
      console.log('✅ 012_audit_events migration already applied');
      ...
      if (tableCheck.rows[0].count > 0) {
        ...
        const sql = fs.readFileSync(
          path.join(__dirname, 'src', 'migrations', '012_audit_events.sql'), 
          'utf-8'
        );
      ...
      console.log('⏳ Applying 012_audit_events migration...');
      await pool.query(sql);
      await pool.query(
