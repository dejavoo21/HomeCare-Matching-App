...
export function normalizeFollowUpRequired(record: any) {
  return Boolean(record?.follow_up_required ?? record?.followUpRequired ?? false);
}

export function normalizeAdminFollowUpScheduled(record: any) {
  return Boolean(
    record?.admin_follow_up_scheduled ?? record?.adminFollowUpScheduled ?? false
  );
}
